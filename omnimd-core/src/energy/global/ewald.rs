...
use crate::{Array3, Complex, Matrix3, Vector3D};
use crate::{CellShape, Configuration, UnitCell};
...
fn compute_ewald_factors(
    factors: &mut EwaldFactorVec,
    cell: &UnitCell,
...
fn compute_ewald_factors_triclinic(
    factors: &mut EwaldFactorVec,
    cell: &UnitCell,
    parameters: &EwaldParameters,
) {
...
///
/// ```
/// # use omnimd_core::sys::{Particle, Molecule, UnitCell, System};
/// # use omnimd_core::energy::{Ewald, SharedEwald};
/// # use omnimd_core::types::Vector3D;
...
impl Ewald {
    ...
    pub fn with_accuracy(cutoff: f64, accuracy: f64, configuration: &Configuration) -> Ewald {
        ...
        let alpha = accuracy * f64::sqrt(natoms * cutoff * lengths[0] * lengths[1] * lengths[2])
            / (2.0 * q2);
        ...
        let error = |kmax| {
            ...
            FRAC_2_SQRT_PI * q2 * alpha / min_length / f64::sqrt(kmax * natoms)
                * f64::exp(-arg * arg)
        };
...
impl Ewald {
    ...
    fn self_energy(&self, configuration: &Configuration) -> f64 {
        let q2 = configuration.particles().charge.iter().map(|q| q * q).sum::<f64>();
...
impl Ewald {
    ...
    fn eik_dot_r(&mut self, configuration: &Configuration) {
        ...
        for spatial in 0..3 {
            for k in 2..(self.kmax + 1) {
                for i in 0..natoms {
                    self.eikr[(k, spatial, i)] =
                        self.eikr[(k - 1, spatial, i)] * self.eikr[(1, spatial, i)];
    ...
    fn k_space_forces(&mut self, configuration: &Configuration, forces: &mut [Vector3D]) {
        ...
        self.factors
            ...
            .for_each(|((&(ikx, iky, ikz), factor), rho)| {
                ...
                for i in 0..natoms {
                    let eikr =
                        self.eikr[(ikx, 0, i)] * self.eikr[(iky, 1, i)] * self.eikr[(ikz, 2, i)];
    ...
    fn delta_rho_move_rigid_molecules(
        ...
        let molecule = configuration.molecule(molecule_id);
        let mut new_energy_ikr =
            Ewald3DArray::zeros((-self.kmax..(self.kmax + 1), 3, molecule.size()));
        ...
        for spatial in 0..3 {
            for k in 2..(self.kmax + 1) {
                for i in 0..molecule.size() {
                    new_energy_ikr[(k, spatial, i)] =
                        new_energy_ikr[(k - 1, spatial, i)] * new_energy_ikr[(1, spatial, i)];
    ...
    fn k_space_move_molecule_cost(
        ...
        let delta_rho =
            self.delta_rho_move_rigid_molecules(configuration, molecule_id, new_positions);
...
impl SharedEwald {
    ...
    /// # Example
    /// ```
    /// # use omnimd_core::energy::{Ewald, SharedEwald, CoulombicPotential};
...
mod tests {
    ...
    mod molecules {
        use super::*;
        use crate::Vector3D;
        use crate::{CoulombicPotential, GlobalPotential, PairRestriction};
    ...
    fn move_molecule() {
        ...
        #[allow(clippy::unreadable_literal)]
        let check_cache = |mut system: System,
                           ewald: Ewald,
                           compute_energy: EnergyCompute,
                           compute_cost: CostCompute| {
        ...
        check_cache(
            ...
            |ewald, system| ewald.read().real_space_energy(system),
        ...
        check_cache(
            ...
            |ewald, system| ewald.write().k_space_energy(system),
            |ewald, system, molecule, positions| {
        ...
        check_cache(
            system,
            Ewald::new(8.0, 10, None),
            |ewald, system| ewald.energy(system),
            |ewald, system, molecule, positions| {
