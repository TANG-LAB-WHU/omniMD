...
use omnimd::compute::{AtomicVirial, Compute, Forces, MolecularVirial, PotentialEnergy};
use omnimd::EnergyCache;

mod utils;

fn energy_computation(c: &mut Criterion) {
    let system = utils::get_system("propane");
    c.bench_function("propane::energy", move |b| {
        b.iter(|| {
    ...
    let system = utils::get_system("propane");
    c.bench_function("propane::force", move |b| {
        b.iter(|| {
    ...
    let system = utils::get_system("propane");
    c.bench_function("propane::atomic_virial", move |b| {
        b.iter(|| {
    ...
    let system = utils::get_system("propane");
    c.bench_function("propane::molecular_virial", move |b| {
        b.iter(|| {
...
fn monte_carlo_cache(c: &mut Criterion) {
    ...
    c.bench_function("propane::move_molecule_cost", move |b| {
        b.iter_batched(
    ...
    c.bench_function("propane::move_all_molecules_cost", move |b| {
        b.iter_batched_ref(
