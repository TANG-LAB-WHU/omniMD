...
use omnimd::{CoulombicPotential, Ewald, GlobalPotential, PairRestriction, SharedEwald, Wolf};
...
fn ewald_energy_computation(c: &mut Criterion) {
    ...
    let ewald = get_ewald();
    c.bench_function("water::ewald::energy", move |b| {
        b.iter(|| {
    ...
    let system = utils::get_system("water");
    let ewald = get_ewald();
    c.bench_function("water::ewald::force", move |b| {
        b.iter_batched_ref(
    ...
    let system = utils::get_system("water");
    let ewald = get_ewald();
    c.bench_function("water::ewald::atomic_virial", move |b| {
        b.iter(|| {
    ...
    let system = utils::get_system("water");
    let ewald = get_ewald();
    c.bench_function("water::ewald::molecular_virial", move |b| {
        b.iter(|| {
...
fn ewald_monte_carlo_cache(c: &mut Criterion) {
    ...
    c.bench_function("water::ewald::move_molecule_cost", move |b| {
        b.iter_batched(
    ...
    c.bench_function("water::ewald::move_all_molecules_cost", move |b| {
        b.iter_batched_ref(
...
fn wolf_energy_computation(c: &mut Criterion) {
    let system = utils::get_system("water");
    let wolf = get_wolf();
    c.bench_function("water::wolf::energy", move |b| {
        b.iter(|| {
    ...
    let system = utils::get_system("water");
    let wolf = get_wolf();
    c.bench_function("water::wolf::force", move |b| {
        b.iter_batched_ref(
            || vec![Vector3D::zero(); system.size()],
    ...
    let system = utils::get_system("water");
    let wolf = get_wolf();
    c.bench_function("water::wolf::atomic_virial", move |b| {
        b.iter(|| {
    ...
    let system = utils::get_system("water");
    let wolf = get_wolf();
    c.bench_function("water::wolf::molecular_virial", move |b| {
        b.iter(|| {
...
fn wolf_monte_carlo_cache(c: &mut Criterion) {
    ...
    c.bench_function("water::wolf::move_molecule_cost", move |b| {
        b.iter_batched(
            || utils::move_rigid_molecule(&system),
            |(molid, positions)| cache.move_molecule_cost(&system, molid, &positions),
            BatchSize::SmallInput,
    ...
    c.bench_function("water::wolf::move_all_molecules_cost", move |b| {
        b.iter_batched_ref(
            || utils::move_all_rigid_molecule(system.clone()),
            |system| cache.move_all_molecules_cost(system),
            BatchSize::SmallInput,
