...
use omnimd_core::{Matrix3, System, Vector3D};
...
impl Integrator for VelocityVerlet {
    ...
    fn integrate(&mut self, system: &mut System) {
        ...
        for (position, velocity, acceleration) in
            soa_zip!(system.particles_mut(), [mut position, mut velocity], &self.accelerations)
        {
        ...
        for (&mass, acceleration, force) in
            soa_zip!(system.particles(), [mass], &mut self.accelerations, forces)
        {
        ...
        for (velocity, acceleration) in
            soa_zip!(system.particles_mut(), [mut velocity], &self.accelerations)
        {
            *velocity += 0.5 * dt * acceleration;
        }
    }
...
impl Integrator for Verlet {
    fn setup(&mut self, system: &System) {
        ...
        for (position, velocity, prevpos) in
            soa_zip!(system.particles(), [position, velocity], &mut self.prevpos)
        {
    ...
    fn integrate(&mut self, system: &mut System) {
        ...
        for (position, velocity, mass, prevpos, force) in soa_zip!(
            system.particles_mut(),
            [mut position, mut velocity, mass],
            &mut self.prevpos,
            forces
        ) {
            // Save positions at t
...
impl Integrator for LeapFrog {
    ...
    fn integrate(&mut self, system: &mut System) {
        ...
        for (position, velocity, acceleration) in
            soa_zip!(system.particles_mut(), [mut position, velocity], &self.accelerations)
        {
            *position += velocity * dt + 0.5 * acceleration * dt2;
        }

        let forces = system.forces();
        for (velocity, &mass, acceleration, force) in
            soa_zip!(system.particles_mut(), [mut velocity, mass], &mut self.accelerations, &forces)
        {
