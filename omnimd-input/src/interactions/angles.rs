...
use omnimd_core::energy::{AnglePotential, DihedralPotential};
use omnimd_core::energy::{CosineHarmonic, Harmonic, Morse, NullPotential, Torsion};
use omnimd_core::System;

use crate::extract;
use crate::{Error, FromToml, InteractionsInput};
...
impl InteractionsInput {
    ...
    pub(crate) fn read_angles(&self, system: &mut System) -> Result<(), Error> {
        let Some(angles) = self.config.get("angles") else {
            return Ok(());
        };

        let angles =
            angles.as_table().ok_or(Error::from("the 'angles' section must be a table"))?;

        for (key, table) in angles {
            let atoms = key.split('-').collect::<Vec<_>>();
            if atoms.len() != 3 {
                return Err(Error::from(format!(
                    "expected three atoms for angle potential, got {} ({:?})",
                    atoms.len(),
                    atoms
                )));
            }
    ...
    pub(crate) fn read_dihedrals(&self, system: &mut System) -> Result<(), Error> {
        let Some(dihedrals) = self.config.get("dihedrals") else {
            return Ok(());
        };

        let dihedrals = dihedrals
            .as_table()
            .ok_or(Error::from("the 'dihedrals' section must be a table"))?;

        for (key, table) in dihedrals {
            let atoms = key.split('-').collect::<Vec<_>>();
            if atoms.len() != 4 {
                return Err(Error::from(format!(
                    "expected four atoms for dihedral potential, got {} ({:?})",
                    atoms.len(),
                    atoms
                )));
            }
