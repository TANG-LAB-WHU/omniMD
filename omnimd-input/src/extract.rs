...
pub fn table<'a>(key: &str, config: &'a Table, context: &str) -> Result<&'a Table, Error> {
    let table = config
        .get(key)
        .ok_or(Error::from(format!("missing '{key}' key in {context}")))?;
    return table
        .as_table()
        .ok_or(Error::from(format!("'{key}' must be a table in {context}")));
}
...
pub fn str<'a>(key: &str, config: &'a Table, context: &str) -> Result<&'a str, Error> {
    ...
    return string
        .as_str()
        .ok_or(Error::from(format!("'{key}' must be a string in {context}")));
}
...
pub fn slice<'a>(key: &str, config: &'a Table, context: &str) -> Result<&'a [Value], Error> {
    ...
    let array = array
        .as_array()
        .ok_or(Error::from(format!("'{key}' must be an array in {context}")));
...
pub fn typ<'a>(config: &'a Table, context: &str) -> Result<&'a str, Error> {
    ...
    return typ
        .as_str()
        .ok_or(Error::from(format!("'type' key must be a string in {context}")));
}
