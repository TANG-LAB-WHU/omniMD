...
fn validate(config: &Table) -> Result<(), Error> {
    let input = config.get("input").ok_or(Error::from("missing 'input' table"))?;

    let version = input
        .get("version")
        .ok_or(Error::from("missing 'version' key in 'input' table"))?;

    let version = version.as_integer().ok_or(Error::from("'input.version' must be an integer"))?;

    if version != 1 {
        return Err(Error::from(format!(
            "can only read version 1 of input, got version {version}"
        )));
    }
    Ok(())
}
